...
- **Accessibility** — keyboard-operable disclosure rows (OS stack, risk matrix), `aria-expanded` state, labeled canvases, a dedicated `role="status"` live region that announces each finished agent reply as a unit (the streaming log itself is not a live region, so word-by-word inserts aren't read as noise), focus returned to the launcher when the drawer closes, a disclosure menu for the nav below 780px, and Escape closes the drawer.
